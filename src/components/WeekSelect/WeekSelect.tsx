...
import { HiChevronLeftOutline, HiChevronRightOutline } from "@qwikest/icons/heroicons";
...
export default component$((props: Props) => {
  ...
  return (
    <div class="flex items-center gap-2">
      <Link href={`/week/${toWeekId(decrementWeek(props.weekId))}`} class="btn  btn-ghost text-3xl">
        <HiChevronLeftOutline />
      </Link>
      <div class="text-lg grow">{getWeekRelative(props.weekId)}</div>
      <Link href={`/week/${toWeekId(incrementWeek(props.weekId))}`} class="btn btn-ghost text-3xl">
